X-Mouse Button Control: Portable Edition.
Copyright (c) 2013-2023 Phillip Gibbons
-----------------------------------------------------------------------------

This file describes how to use X-Mouse Button Control (XMBC) in PORTABLE mode.
...
There are two versions:
   ...
   The 32bit version will work on Windows x64 editions but you will lose some functionality.

   Simply run the relevant executable suitable for your OS.

Additional language packs may become available or get updated. You can find these, along with translations
of the user manual available from http://www.highrez.co.uk/downloads/xmbc_languages.htm. Simply copy the 
language pack (.xmbclp file) and copy it to the same folder as the executable and it should then 
appear in the list of available languages in the settings dialog.


Limitations for the Portable Version of XMBC:
	...
	detect profiles as the mouse moves or properly scroll the window under the cursor until you 
	physically activate a non-elevated window.
	...
	In general, the installed version of XMBC (so long as it is installed to the default location 
	(under Program Files) does not suffer these limitations as it runs as an accessibility tool.
